<?php
/**
 * Script to update articles with structured content for TOC
...
echo "<h1>Actualizando artículos con contenido estructurado para TOC</h1>";
echo "<pre>";
...
if ($article) {
    $article->body = getFeaturedArticleBody();
    $article->save();
    echo "✓ Updated: {$article->title}\n";
}

// Update university article
$article2 = Article::where('slug', 'like', '%unmdp%')->first();
if ($article2) {
    $article2->body = getUniversityArticleBody();
    $article2->save();
    echo "✓ Updated: {$article2->title}\n";
}

// Update opinion article
$article3 = Article::where('slug', 'like', '%opinion%')->first();
if ($article3) {
    $article3->body = getOpinionArticleBody();
    $article3->save();
    echo "✓ Updated: {$article3->title}\n";
}

echo "\n¡Listo! Ahora los artículos tienen contenido con headings para el TOC.\n";
echo "IMPORTANTE: Elimina este archivo después de usarlo.\n";
echo "</pre>";

function getFeaturedArticleBody(): string
{
    return <<<HTML
...
function getUniversityArticleBody(): string
{
    return <<<HTML
...
function getOpinionArticleBody(): string
{
    return <<<HTML
